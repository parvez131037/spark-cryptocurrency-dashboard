...
function ExchangeCoins() {
  ...
  //till here .
    //below are the function converting input data
  function convert() {
    var rate = info[to];
    if(input >= 0)
  ...
  <div className="w-full px-4 xl:px-6 dark:bg-gray-700 dark:text-white">
    ...
    <div className="flex place-content-between xl:place-content-stretch dark:bg-gray-700">
      ...
      <div className="mt-4 xl:mt-5 space-y-1 xl:space-y-3 xl:pl-6 flex flex-col dark:bg-gray-700">
        ...
        <p className="dark:bg-gray-700 text-sm">
          ...
          on='hover'
          repositionOnResize='true'>
            <div className="--Div--5-for-error display text-xs text-black ">The string is not a valid input</div>
          </Popup>
          : null 
        }
        </p>
        ...
        <span className="px-[10px] py-0.8 xl:pt-4 text-sm xl:w-36 text-teal-500 font-medium">
    ...
    <div className=" flex justify-center xl:mt-3 dark:text-white">
      <button
        type="button"
        className="bg-blue-400 text-sm px-1 py-1 sm:px-4 h-10 text-white font-bold rounded-md xl:px-6 xl:py-1 mt-2 transition duration-300 hover:bg-blue-500 focus:outline-none focus:shadow-outline dark:bg-blue-400"
        onClick={(e) => {
          handleButton();
        }} 
      >
